...
struct VideoCompletionView: View {
    ...
    private func loadMedia() {
        ...
        print("[VideoCompletionView] Starting load for: \(url.absoluteString)")
    ...
    private func downloadAndPlay(url: URL) {
        ...
        Task {
            do {
                print("[VideoCompletionView] Step: Preparing Download Session")
                ...
                print("[VideoCompletionView] Step: Executing Download Request")
                ...
                print("[VideoCompletionView] Step: Download Success, saved to: \(localURL.path)")
        ...
        playerObserver = playerItem.observe(\.status, options: [.new, .initial]) { [self] item, _ in
            DispatchQueue.main.async {
                if item.status == .readyToPlay {
                    print("[VideoCompletionView] Step: Player Ready")
                    isPlayerReady = true
                    playerError = nil
                    player.play()
    ...
    private var navBar: some View {
        HStack {
            Button(action: onClose) {
                Image(systemName: "xmark").font(.system(size: 14, weight: .bold)).foregroundColor(.white).frame(width: 44, height: 44).background(Circle().fill(Color.white.opacity(0.1)))
            }
            Spacer()
            Text("SYNTHESIS READY").font(.system(size: 18, weight: .black)).foregroundStyle(AppTheme.accentGradH)
            Spacer()
            Button(action: onShare) {
                Image(systemName: "square.and.arrow.up").font(.system(size: 16, weight: .bold)).foregroundColor(.white).frame(width: 44, height: 44).background(Circle().fill(AppTheme.accentPrimary.opacity(0.2)))
            }
    ...
    private var mediaCard: some View {
        GeometryReader { geo in
            ...
            let h = min(w * 1.6, geo.size.height * 0.6)
            ZStack {
                if let error = playerError {
                    VStack(spacing: 12) {
                        Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 40)).foregroundColor(AppTheme.error)
                        Text(error).font(.system(size: 12)).foregroundColor(AppTheme.textMuted).multilineTextAlignment(.center).padding()
                        Button("Retry") { loadMedia() }.font(.system(size: 14, weight: .bold)).foregroundColor(AppTheme.accentSecondary)
                    }
                } else if isPlayerReady, let player = player {
                    VideoPlayer(player: player).clipShape(RoundedRectangle(cornerRadius: 32))
                } else {
                    VStack(spacing: 16) {
                        ProgressView().tint(AppTheme.accentSecondary).scaleEffect(1.2)
                        Text(isLoadingVideo ? "Downloading..." : "Loading...").font(.system(size: 13)).foregroundColor(AppTheme.textMuted)
                    }
                }
                RoundedRectangle(cornerRadius: 32).stroke(AppTheme.accentGrad, lineWidth: 1.5)
            }.frame(width: w, height: h).frame(maxWidth: .infinity, maxHeight: .infinity)
        }
    }
    ...
    private var actionBar: some View {
        HStack(spacing: 16) {
            ...
            Button(action: handleDownload) {
                HStack(spacing: 12) {
                    if isSavingToGallery { ProgressView().tint(.white) }
                    else { Image(systemName: "arrow.down.circle.fill").font(.system(size: 18, weight: .bold)) }
                    Text(isSavingToGallery ? "SAVING..." : "Save to Vault").font(.system(size: 17, weight: .bold))
                }.foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 72).background(AppTheme.accentGradH).clipShape(RoundedRectangle(cornerRadius: 24))
